...
**当前状态**：CPU MVP 的 P0-P6、MVP 后覆盖阶段 C0-C5 及前八个 C6 增量已实现。从 strict
...
会被保存在 plan metadata 中，但不会让其他 operation 自动变为动态。标准 f32
`MATMUL` 与 `MATMUL_FP8` 还可分别接收 producer 序列化的 external plain INT32
