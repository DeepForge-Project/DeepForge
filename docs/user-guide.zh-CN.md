...
| `CONCATENATE` | 编号输入、非负 axis、无 in-place mode；单个标准 f32 operation 支持有界 input/Y descriptor override |
| `POINTWISE` | v1.24.0 全部 50 个 mode，尾维对齐的 NumPy broadcast |
| `REDUCTION` | 全部 9 个 mode；输入输出 rank 相同，被归约维度为 1；单个标准 f32 operation 支持有界 X/Y descriptor override |
...
单个 `reshape_mode=LOGICAL` 的标准 f32 `RESHAPE` 可 override external plain X/Y
descriptor。X 只读、Y 只写；不支持 virtual、pass-by-value、ragged、reordered、额外
tensor 或组合图。X/Y 的编译 rank 可以不同，但每个 runtime descriptor 必须保持各自
的 rank。假设序列化上界为 X `[2,3,4]`、Y `[4,6]`，一个合法的非连续调用是：
...
最终 X/Y 的正元素总数必须相同，partial override 后也不例外。执行保持 LOGICAL
reshape 顺序：按 lexicographic 顺序读取 X，并按 lexicographic 顺序写入 Y。
Dimension、stride 和 storage span 仍受上述公共序列化上界约束。

单个标准 f32 `REDUCTION` 的全部 9 个已支持 mode 都可 override external plain X/Y
descriptor。X 只读、Y 只写且二者保持相同的编译 rank；不支持 virtual、pass-by-value、
ragged、reordered、额外 tensor 或组合图。归约 axis 由序列化最大 shape 冻结：编译 X/Y
extent 不同时，该 axis 为归约 axis 且编译 Y extent 必须为 1。Runtime Y 在归约 axis
仍为 1，runtime X 可缩小；每个保留 axis 的最终 X/Y extent 必须相同。假设序列化 X
为 `[2,3,4]`、Y 为 `[2,1,4]`，一个合法调用是：
...
单个标准 f32 `CONCATENATE` 可 override 1 至 63 个 external plain input descriptor 和
external plain Y。Input 只读、Y 只写，全部 role 保持相同的编译 rank 1 至 64，序列化
非负 axis 固定不变。Role UID 必须不同，input port 必须连续编号为 `0..N-1`；不支持 in-place、virtual、
pass-by-value、ragged、reordered、额外 tensor 或组合图。三个 input 在 axis 1 上 concat
时，一个合法的非连续调用是：
...
每个非 concat axis 上，各最终 input extent 必须等于 Y；固定 concat axis 上，全部最终
...
override policy 只需要设置 override flag。
