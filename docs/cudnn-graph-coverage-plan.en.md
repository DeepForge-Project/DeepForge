...
- `.dfo` format version 2 containing a generic tensor/argument table and
  ...
  roles, and fixed axis. Version 12 adds the producer-specific block-scale
  MATMUL override policy and ordered A/SF_A/B/SF_B/C roles while retaining
...
ragged-sequence artifacts, plus format-v6 MATMUL-override, format-v7
  ...
  also read format-v9 REDUCTION-override, format-v10 TRANSPOSE-override, and
  format-v11 CONCATENATE-override artifacts and write format v12. Unknown
...
artifact reload, malformed metadata, Release, ASan, and UBSan. The fourth adds
ragged forward row outputs and backward tensors, validates backward max-total
hints, supports independently compacted K/V page tables, and implements
standard-f32 forward block masks plus forward/backward sink tokens and dSink.
...
set.
The sixteenth implements bounded descriptor overrides for the exact
producer-specific rank-3 graph with two block-scale dequantize operations
feeding one MATMUL. External A/B are FP4 E2M1, SF_A/SF_B are reordered
F8_128x4 E4M3, C is BFLOAT16, and the dequantized FLOAT tensors are virtual.
Runtime B/M/N/K relations, K divisibility by 16, 128x4 scale padding, and all
five producer-canonical strides are validated together. Artifact v12 persists
A/SF_A/B/SF_B/C roles. Dynamic packed/reordered addressing, independent
virtual views, BFLOAT16 output, maximum and partial overrides, reload,
malformed relations/layouts/graphs, Release, ASan, and UBSan form its
acceptance set.
...
producer-specific block-scale MATMUL, and SDPA-forward descriptor
