...
编译期错误：schema/version、超出 exact-pointwise override policy 的动态行为、
layout、dtype、未知 node、unsupported access、无法 workspace-plan、非法
vectorization 或最终 dialect residue。

运行期回退只发生在 CPU feature：AVX-512 -> AVX2 -> scalar。它不掩盖输入契约
错误；只有按 C6 override policy 编译的图可提供经过校验的 runtime dimension 和
stride。
