...
Policies `0` and `1` require an empty role list. Versions `1` through `5`
cannot encode policy `2` and are read with an empty role list.

Version `7` adds policy `3` for one dense standard-f32 SDPA forward operation.
It stores distinct Q, K, V, O UIDs followed, when present, by Stats, Max, and
Sum_exp in that order. Runtime descriptors may change only B, Sq, and Skv;
heads and embedding dimensions stay equal to their compiled values. Reloaded
artifacts validate Q/K/V/O batch, sequence, embedding, GQA, and optional
`[B,Hq,Sq,1]` row-output relations before dispatch. Version `6` cannot encode
policy `3` and rejects it while preserving policy `2` role metadata.

Version `8` adds policy `4` for one standard-f32 logical RESHAPE operation. It
stores exactly two distinct UIDs in X, Y order. X and Y keep their independently
compiled ranks, which may differ, while runtime dimensions and strides may
change within each serialized dimension and byte-span bound. Reloaded artifacts
require the resolved positive X and Y element counts to be equal before
...
Version `10` adds policy `6` for one standard-f32 TRANSPOSE operation. It stores
distinct X, Y role UIDs plus the serialized permutation as an output-axis to
input-axis map. X and Y keep the same fixed rank. Every complete or partial
runtime override must preserve `Y[i] == X[permutation[i]]`; dimensions and
positive non-overlapping strides remain bounded by each serialized descriptor's
...
format version. Versions `1` through `9` have frozen field order and semantics
and are read-only. New compilations write version `10`.
